...
| 15 | source_type | VARCHAR(20) | YES | - | - | 라이선스 소스 (PAYMENT/REDEEM/ADMIN) |
| 16 | source_redeem_id | UUID | YES | - | - | 리딤 발급 시 redemption ID |
| 17 | created_at | TIMESTAMP | NO | CURRENT_TIMESTAMP | - | 생성일시 |
| 18 | updated_at | TIMESTAMP | NO | CURRENT_TIMESTAMP | - | 수정일시 |
...
리딤 코드 발급 단위인 캠페인을 관리하는 테이블

| No | 컬럼명 | 데이터 타입 | NULL | 기본값 | PK/FK | 설명 |
|----|--------|-------------|------|--------|-------|------|
| 1 | id | UUID | NO | uuid_generate_v4() | PK | 기본키 |
| 2 | name | VARCHAR(200) | NO | - | - | 캠페인명 |
| 3 | description | TEXT | YES | - | - | 캠페인 설명 |
| 4 | product_id | UUID | NO | - | FK → products.id | 상품 ID |
| 5 | license_plan_id | UUID | NO | - | FK → license_plans.id | 라이선스 플랜 ID |
| 6 | usage_category | VARCHAR(30) | NO | 'COMMERCIAL' | - | 사용 용도 |
| 7 | seat_limit | INT | YES | - | - | 발급 한도 (NULL=무제한) |
| 8 | seats_used | INT | NO | 0 | - | 현재 발급 수 |
| 9 | per_user_limit | INT | NO | 1 | - | 사용자당 한도 |
| 10 | status | VARCHAR(20) | NO | 'ACTIVE' | - | 캠페인 상태 |
| 11 | valid_from | TIMESTAMP | YES | - | - | 유효 시작일 |
| 12 | valid_until | TIMESTAMP | YES | - | - | 유효 종료일 |
| 13 | created_by | UUID | YES | - | FK → users.id | 생성자 (관리자) |
| 14 | created_at | TIMESTAMP | NO | CURRENT_TIMESTAMP | - | 생성일시 |
| 15 | updated_at | TIMESTAMP | NO | CURRENT_TIMESTAMP | - | 수정일시 |
...
리딤 코드를 관리하는 테이블 (SHA-256 해시만 저장)

| No | 컬럼명 | 데이터 타입 | NULL | 기본값 | PK/FK | 설명 |
|----|--------|-------------|------|--------|-------|------|
| 1 | id | UUID | NO | uuid_generate_v4() | PK | 기본키 |
| 2 | campaign_id | UUID | NO | - | FK → redeem_campaigns.id | 캠페인 ID |
| 3 | code_hash | VARCHAR(64) | NO | - | UNIQUE | SHA-256 해시 |
| 4 | code_type | VARCHAR(10) | NO | 'RANDOM' | - | 코드 타입 |
| 5 | max_redemptions | INT | NO | 1 | - | 최대 사용 횟수 |
| 6 | current_redemptions | INT | NO | 0 | - | 현재 사용 횟수 |
| 7 | is_active | BOOLEAN | NO | TRUE | - | 활성 여부 |
| 8 | expires_at | TIMESTAMP | YES | - | - | 코드 만료일 |
| 9 | created_at | TIMESTAMP | NO | CURRENT_TIMESTAMP | - | 생성일시 |
...
| 값 | 설명 |
|----|------|
| RANDOM | 자동 생성 (16자 영숫자) |
| CUSTOM | 관리자 지정 |

> ON DELETE CASCADE: 캠페인 삭제 시 코드도 함께 삭제

---

### 27. redeem_redemptions (리딤 감사 로그 테이블)

리딤 코드 사용 이력을 기록하는 감사/추적 로그 테이블

| No | 컬럼명 | 데이터 타입 | NULL | 기본값 | PK/FK | 설명 |
|----|--------|-------------|------|--------|-------|------|
| 1 | id | UUID | NO | uuid_generate_v4() | PK | 기본키 |
| 2 | code_id | UUID | NO | - | FK → redeem_codes.id | 코드 ID |
| 3 | campaign_id | UUID | NO | - | FK → redeem_campaigns.id | 캠페인 ID |
| 4 | user_id | UUID | NO | - | FK → users.id | 사용자 ID |
| 5 | license_id | UUID | YES | - | FK → licenses.id | 발급된 라이선스 ID |
| 6 | redeemed_at | TIMESTAMP | NO | CURRENT_TIMESTAMP | - | 사용 일시 |
| 7 | ip_address | VARCHAR(45) | YES | - | - | 사용자 IP |
| 8 | user_agent | VARCHAR(500) | YES | - | - | User-Agent |

---

### 28. redeem_user_campaign_counters (사용자별 캠페인 카운터 테이블)

사용자별 캠페인 사용 횟수를 관리하는 테이블 (per_user_limit 원자적 enforce)

| No | 컬럼명 | 데이터 타입 | NULL | 기본값 | PK/FK | 설명 |
|----|--------|-------------|------|--------|-------|------|
| 1 | id | UUID | NO | uuid_generate_v4() | PK | 기본키 |
| 2 | user_id | UUID | NO | - | FK → users.id | 사용자 ID |
| 3 | campaign_id | UUID | NO | - | FK → redeem_campaigns.id | 캠페인 ID |
| 4 | count | INT | NO | 0 | - | 사용 횟수 |

**제약 조건:**
- UNIQUE(user_id, campaign_id): 사용자+캠페인 조합 유니크
