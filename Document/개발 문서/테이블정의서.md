...
| 8 | products | 상품 종류 테이블 | 결제 |
| 9 | price_plans | 상품 가격 테이블 | 결제 |
| 10 | promotions | 프로모션/쿠폰 테이블 | 결제 |
| 11 | subscriptions | 유저 구독 관리 테이블 | 결제 |
| 12 | billing_keys | 빌링키 테이블 (자동결제용) | 결제 |
| 13 | subscription_payments | 구독 결제 이력 테이블 | 결제 |
| 14 | payments | 결제 테이블 | 결제 |
| 15 | payment_details | 결제 상세 테이블 | 결제 |
| 16 | activity_logs | 활동 로그 테이블 | 로그 |
| 17 | user_change_logs | 유저 정보 변경 로그 테이블 | 로그 |
| 18 | admin_logs | 관리자 로그 테이블 | 로그 |
| 19 | license_plans | 라이선스 플랜 테이블 | 라이선스 |
| 20 | license_plan_entitlements | 플랜 기능 권한 테이블 | 라이선스 |
| 21 | licenses | 라이선스 테이블 | 라이선스 |
| 22 | license_activations | 라이선스 활성화 테이블 | 라이선스 |
| 23 | revoked_offline_tokens | 무효화된 오프라인 토큰 테이블 | 라이선스 |
...
| No | 컬럼명 | 데이터 타입 | NULL | 기본값 | PK/FK | 설명 |
|----|--------|-------------|------|--------|-------|------|
| 1 | email | VARCHAR(255) | NO | - | PK | 이메일 (로그인 ID) |
| 2 | password_hash | VARCHAR(255) | YES | - | - | 비밀번호 해시 (소셜 로그인은 NULL) |
| 3 | roles_code | VARCHAR(10) | NO | '002' | FK → user_roles.code | 역할 코드 |
| 4 | name | VARCHAR(100) | YES | - | - | 이름 (결제 시 입력) |
| 5 | phone | VARCHAR(20) | YES | - | - | 전화번호 (결제 시 입력) |
| 6 | country_code | VARCHAR(10) | YES | 'KR' | FK → countries.code | 국가 코드 |
| 7 | is_active | BOOLEAN | NO | TRUE | - | 계정 활성화 상태 |
| 8 | deactivated_at | TIMESTAMP | YES | - | - | 계정 비활성화 시점 |
...
| 2 | user_email | VARCHAR(255) | NO | - | FK → users.email | 유저 이메일 |
...
| 3 | user_email | VARCHAR(255) | NO | - | - | 토큰 소유자 이메일 |
...
|----|--------|-------------|------|--------|-------|------|
| 1 | id | BIGINT | NO | AUTO | PK | 기본키 |
| 2 | user_email | VARCHAR(255) | YES | - | FK → users.email | 유저 이메일 |
...
**인덱스**
- `idx_billing_keys_user_email`: user_email
- `idx_billing_keys_is_active`: is_active

---

### 13. subscription_payments (구독 결제 이력 테이블)
...
|----|--------|-------------|------|--------|-------|------|
| 1 | id | BIGINT | NO | AUTO | PK | 기본키 |
| 2 | user_email_fk | VARCHAR(255) | YES | - | FK → users.email | 유저 FK |
...
### 17. user_change_logs (유저 정보 변경 로그 테이블)
...
|----|--------|-------------|------|--------|-------|------|
| 1 | id | BIGINT | NO | AUTO | PK | 기본키 |
| 2 | admin_email | VARCHAR(255) | NO | - | FK → users.email | 관리자 이메일 |
...
### 20. license_plan_entitlements (플랜 기능 권한 테이블)
...
### 22. license_activations (라이선스 활성화 테이블)
...
### 23. revoked_offline_tokens (무효화된 오프라인 토큰 테이블)
