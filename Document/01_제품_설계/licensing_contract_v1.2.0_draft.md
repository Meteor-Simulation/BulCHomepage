...
| 항 | 수단 | 판정 | 효과 |
|---|---|---|---|
| **㉮** | **진단·CI 대조용 공개키 엔드포인트** (평문 — 클라이언트 신뢰 경로 무접촉) | ✅ v1.2 포함 | CI 가 시크릿 0 으로 「내장 상수 ↔ 운영 공개키」 를 대조. 이번 사고 유형(임베딩 키 불일치) 자동 검출 + 운영 공개키 식별 |
| **㉯** | 클라이언트 **내장 키 배열**(`kid → key`) + 토큰 `header.kid` 로 선택 | ✅ v1.2 포함 | 양 키 병존 배포 → 서버 전환 → 구키 제거. 무중단 회전 1회 가능 |
| **㉰** | KSK-서명 JWKS 런타임 배포 (`GET /.well-known/jwks.json` 을 **키 서명 전용 내장 루트키**로 서명) | ⏸ 이연 — 회전 실수요 실증 시 | 재배포 없는 회전. 전거 = TUF / DNSSEC 2계층 |

> **근거 — 원 설계와 업계 관행이 같은 방향이다.**
> ① 원 스펙 `bulc_auth_module_spec_v0.4.md` §8.1 은 공개키 **내장(고정)** 을 명시하고 키 회전을 "v0.2-min 미고려 (**추후 multi-key 지원**)" 으로 의도적 이연해 두었다 — ㉯ 는 설계 변경이 아니라 **§8.1 이연분의 이행**이다.
> ② 오프라인 검증을 하는 최종사용자 소프트웨어의 업계 관행 = 검증 키 내장. Keygen 은 런타임 로드를 "공격자가 키를 바꿔치기해 라이선스를 우회한다"는 사유로 **명문 금지**한다. JWKS 런타임 fetch 는 OIDC *서버측 RP* 의 관행이며, 사용자 통제 머신의 오프라인 검증기에는 해당하지 않는다.
> ③ ㉮ 는 `licensing_api.md` Appendix B 의 `LICENSE_RSA_PUBLIC_KEY` **"클라이언트 배포용"** 슬롯 의도의 실현이다 — 슬롯은 스펙에 있으나 배포 수단(엔드포인트)은 어디에도 없었고, 구현은 그 슬롯 자체를 읽지 않는다.

> ⚠️ **부수 drift (사본 단일화 목록행 추가 — MDP-794)**: 스펙 Appendix B 설정 스키마(`bulc.licensing.rsa-keys.private-key` / `public-key`) ↔ 구현(`bulc.licensing.private-key-base64` / `private-key-path`) 불일치. `public-key` 슬롯은 미구현 — 구현은 개인키에서 공개키를 유도하며 노출 표면이 없다.
...
**해소 경로 (2026-09-02 확정 — 시크릿 0 구성)**: 키 정합 검사에 비밀키는 어느 단계에도 불필요하다. 신규 키 쌍 발급·운영 개인키 복구도 불요.

1. **주 = ㉮ 엔드포인트 대조**: CI 가 운영 공개키를 fetch 해 각 클라이언트 내장 상수·`kid` 와 대조.
2. **보조 = 만료 토큰 픽스처**: 운영 키로 서명된 *만료* 토큰을 리포에 커밋(만료 토큰 = 자격증명 아님 · 더미 라이선스로 발급 권장). 검증기가 **서명을 만료보다 먼저** 검사하므로 판별된다 — Rust 실측: 키 불일치 = `token signature verification failed`(`license.rs:430`) / 키 일치+만료 = `license token expired…`(`:442-446`). 네트워크 차단 환경 폴백 겸 검증기 자체 회귀.
3. 즉시 일회성 진단은 기존 하네스로 가능: `cd GPU_code && BULC_TEST_TOKEN=<운영 JWT> cargo test real_token_signature_e2e -- --ignored --nocapture`

→ MDP-788 (㉮·㉯ + `kid` 도출) · MDP-801 (공통 적합성 검사 이관 — 방식 확정 · 착수 대기)
...
| MDP-801 | 작업 | 키 정합 검증 하네스 SDK 이관 — 양성 경로 회귀 + 공통 적합성 검사 **(방식 확정 · 착수 대기)** | sdk | 1~2 |

합계 **33~52 인일** (MDP-801 제외 · ㉮ 엔드포인트 0.5~1 은 MDP-788 범위에 흡수). KETI CLI 납품 최단 경로는 MDP-792 · 794(부분) · 795 · 796 · 797 로 약 **15~25 인일**이며, MDP-788(키 배포)은 CLI 배포 *이전*에 끝나야 한다 — 배포 후에는 공개키가 박힌 바이너리가 현장에 풀린다.
...
| KSK-서명 JWKS **런타임** 키 배포 (§2 ㉰) | 업계 관행(오프라인 검증기 = 키 내장, Keygen 명문 금지 사유 참조)과 원 스펙 §8.1 방향에 따라 이연. ㉮+㉯ 로 당면 요구(공개키 식별 + 무중단 회전 1회) 충족. 회전 실수요 실증 시 TUF/DNSSEC 2계층으로 재개봉 |
