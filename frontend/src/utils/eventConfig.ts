...
/** 사은품 이벤트 대상 언어 (한국어 사용자 전용). IP 기반 자동 감지 + 사용자 명시 설정 모두 반영 */
export const BOOTH_GIFT_TARGET_LANGUAGE = 'ko';
...
/** 언어 코드가 사은품 이벤트 대상인지 판정 (한국어만 허용) */
export const isLanguageEligibleForBoothGift = (
  language: string | null | undefined
): boolean => language === BOOTH_GIFT_TARGET_LANGUAGE;

/** 활성 상태 + 사용자 언어 적격성 종합 판정 */
export const isBoothGiftEligible = (
  config: BoothGiftEventConfig | null,
  language: string | null | undefined
): boolean => isBoothGiftActive(config) && isLanguageEligibleForBoothGift(language);
