...
const SignupModal: React.FC<SignupModalProps> = ({ isOpen, onClose, onSwitchToLogin }) => {
  ...
  // 소셜 로그인/회원가입 핸들러 (LoginModal과 동일한 OAuth 엔드포인트 사용)
  const handleSocialLogin = (provider: string) => {
    window.location.href = `${getApiBaseUrl()}/api/auth/oauth2/authorize/${provider}`;
  };
    ...
    <div className="modal-overlay" onMouseDown={handleOverlayMouseDown}>
      <div className="modal-content signup-modal">
        ...
        <div className="modal-social">
          <p className="modal-social-title">{t('signup.socialSignup')}</p>
          <div className="modal-social-btns">
            {!isEnglish && (
              <>
                <button type="button" className="social-btn naver" onClick={() => handleSocialLogin('naver')} disabled={isLoading}>
                  <svg viewBox="0 0 24 24" className="social-icon">
                    <path d="M16.273 12.845L7.376 0H0v24h7.727V11.155L16.624 24H24V0h-7.727v12.845z" fill="currentColor"/>
                  </svg>
                </button>
                <button type="button" className="social-btn kakao" onClick={() => handleSocialLogin('kakao')} disabled={isLoading}>
                  <svg viewBox="0 0 24 24" className="social-icon">
            ...
            <button type="button" className="social-btn google" onClick={() => handleSocialLogin('google')} disabled={isLoading}>
              <svg viewBox="0 0 24 24" className="social-icon">
