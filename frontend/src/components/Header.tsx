...
interface HeaderProps {
  ...
  /** true면 헤더 우측 인증 영역 전체(로그인/회원가입/내 정보)를 숨김 */
  hideAuth?: boolean;
  /** true면 서브네비 사용 시에도 플로팅 "문의" 버튼을 숨김 */
  hideContact?: boolean;
}
    ...
    <>
      ...
      {showSubNav && !hideContact && (
        <button className="floating-contact-btn" onClick={() => setContactModalOpen(true)}>
          ...
          <span className="floating-contact-text">{contactLabel || t('header.contact')}</span>
        </button>
      )}
