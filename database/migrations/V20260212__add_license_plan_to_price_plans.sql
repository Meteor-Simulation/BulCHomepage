...
-- 실행일: 2026-02-09 (운영 DB 적용 완료)
-- 설명: 결제 완료 시 자동 라이선스 발급을 위한 연결
--
-- ⚠️ 동기화 주의: 이 파일의 INSERT/UPDATE는 init.sql에도 동일 내용이 반영되어 있다.
--   값 변경 시 양쪽 모두 수정하거나, 새 마이그레이션 파일(V<날짜>__...)을 추가할 것.
-- 참고: 원래 V002로 작성됐다가 날짜 기반 명명 통일을 위해 2026-05-07 rename됨.
--   Flyway 미사용 환경이라 파일명 변경이 운영 DB에 영향 없음.
-- =========================================================
