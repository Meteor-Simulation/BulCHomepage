...
-- =========================================================
-- 3-1-2. login_attempts (로그인 시도 횟수 제한 테이블)
-- =========================================================
...
COMMENT ON COLUMN login_attempts.email IS '추적 대상 이메일 (UNIQUE)';
COMMENT ON COLUMN login_attempts.attempt_count IS '15분 윈도우 내 실패 횟수';
COMMENT ON COLUMN login_attempts.first_attempt_at IS '현재 윈도우 시작 시각';
COMMENT ON COLUMN login_attempts.locked_until IS '잠금 만료 시각 (NULL이면 미잠금, 5회 실패 시 30분 잠금)';
...
INSERT INTO price_plans (product_code, name, description, price, currency) VALUES
    ...
    ('001', 'BUL:C 3D Premium', '영구 라이선스', 30000000, 'KRW');
...
INSERT INTO license_plan_entitlements (id, plan_id, entitlement_key) VALUES
    ...
    (uuid_generate_v4(), 'b2c3d4e5-f6a7-4b5c-9d0e-1f2a3b4c5d6e', 'evac');
...
-- USER 중복 구매 차단용 partial unique index (최종 방어선).
-- ORG는 다중 라이선스 허용 정책이므로 제외, TRIAL도 무료 체험과 유상 구매가 공존할 수 있도록 제외.
-- 동일 사용자가 동일 제품에 ACTIVE/PENDING 유상 라이선스를 둘 이상 가질 수 없게 강제.
CREATE UNIQUE INDEX uk_licenses_user_product_active_paid
...
-- =========================================================
-- popups (관리자 등록 팝업 - 홈 진입/로그인 직후 표시)
-- =========================================================
CREATE TABLE popups (
    id              BIGINT PRIMARY KEY GENERATED ALWAYS AS IDENTITY,
...
CREATE INDEX idx_popups_priority ON popups(priority);
CREATE INDEX idx_popups_schedule ON popups(start_at, end_at);
...
-- =========================================================
-- lead_contacts (회원가입 없이 외부에서 수집한 메일링 컨택)
-- 동기화: V20260604__create_lead_contacts_table.sql
-- =========================================================
...
CREATE INDEX idx_lead_contacts_company ON lead_contacts(company_name);
CREATE INDEX idx_lead_contacts_unsubscribed_at ON lead_contacts(unsubscribed_at);
CREATE INDEX idx_lead_contacts_source_event ON lead_contacts(source_event);
